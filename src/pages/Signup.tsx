...
export default function Signup() {
  ...
  const mockSignup = (emailVal: string, nameVal: string) => {
    setLoading(true)
    setTimeout(() => {
      localStorage.setItem('dipperai_user', JSON.stringify({ email: emailVal, name: nameVal || 'Zach' }))
      navigate('/dashboard')
    }, 500)
  }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    mockSignup(email, name)
  }

  const handleGoogle = () => {
    mockSignup('zach@example.com', 'Zach')
  }
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center px-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 mt-2">Start building AI agents for free</p>
        </div>
